...
#include "utils.h"
#include <math.h>
#include <stdio.h> // Added this line to include necessary file operation functions
#include <stdlib.h>
#include <string.h>
...
char get_ascii_char(float intensity, int is_edge, int edge_direction) {
  if (is_edge) {
    return EDGE_CHARS[edge_direction];
  } else {
...
ASCIIArt convert_to_ascii(const Image *image, const Image *edges,
                          int ascii_width) {
  ASCIIArt ascii_art;
  ascii_art.width = ascii_width;
  ascii_art.height =
      (int)((float)image->height / image->width * ascii_width * 0.5f);
  ascii_art.data = (char *)safe_malloc(ascii_art.width * ascii_art.height +
                                       ascii_art.height + 1);
  ...
  for (int y = 0; y < ascii_art.height; y++) {
    for (int x = 0; x < ascii_width; x++) {
      ...
      ascii_art.data[data_index++] =
          get_ascii_char(intensity, is_edge, edge_direction);
    }
    ascii_art.data[data_index++] = '\n';
  }
