...
ASCIIArt convert_to_ascii(const Image* image, const Image* edges, int ascii_width) {
    ...
    ascii_art.data = (char*)safe_malloc(ascii_art.width * ascii_art.height + ascii_art.height + 1);
    ...
    for (int y = 0; y < ascii_art.height; y++) {
        for (int x = 0; x < ascii_width; x++) {
            ...
            ascii_art.data[data_index++] = get_ascii_char(intensity, is_edge, edge_direction);
        }
